...
static ColorCorrectionParams ResolveAutoColorParams(PF_InData* in_data, PF_ParamDef* const params[])
{
    ...
    PF_Pixel8   *outP)
{
    ...
    if (params->saturation != 100.0f) {
        float updated_luma = Rec709Luma(r, g, b);
    ...
    PF_LayerDef     *output)
{
    ...
    // 16. Analysis Confidence Slider (disabled until host-side UI updates are wired)
    AEFX_CLR_STRUCT(def);
    PF_ADD_FLOAT_SLIDERX_DISABLED(STR(StrID_Confidence_Param_Name), 0, 100, 0, 100, 100,
